const airtableFetch = async (table) => {
  ...
  try {
    const response = await fetch(
      `https://api.airtable.com/v0/${BASE_ID}/${table}?view=Grid%20view&pageSize=${PAGE_SIZE}`,
      {
        headers: {
    ...
    const recursiveFetch = async (offset) => {
      ...
      const response = await fetch(
        `https://api.airtable.com/v0/${BASE_ID}/${table}?view=Grid%20view&offset=${offset}&pageSize=${PAGE_SIZE}`,
        {
          headers: {
            Authorization: `Bearer ${process.env.AIRTABLE_API_KEY}`,
          },
